...
const DiagramBoard = () => {
    ...
    //nodes
    const initialNodes = [
        { id: '1', type: 'background', position: { x: 0, y: 0 }, data: { years: years, zoom: zoom, distanceBetweenYears: distanceBetweenYears }, draggable: false, selectable: false },
        { id: '2', type: 'singleNode',position: { x: 100, y: 100 }, data: {}, draggable: false },
        { id: '3', type: 'groupNode',position: { x: 400, y: 700 }, data: {}, draggable: false },
    ];
    ...
    return (
        <div className={`${isDarkMode ? "dark" : "light"} w-screen h-screen`}>
            <ReactFlow nodes={initialNodes} edges={initialEdges} nodeTypes={nodeTypes} colorMode={colorMode}
                       nodeExtent={extent} translateExtent={extent}
            ...
            {/*Dynamic time axis*/}
            <div className={`fixed w-full bg-black_text dark:bg-white_text h-[1px] top-3 text-black_text dark:text-white_text transition`}>
                {years.map((year, index) => (
                    ...
                    >
                        <div className="w-2 h-2 bg-black_text dark:bg-white_text rounded-full transition" style={{transform: `scale(${zoom})`}}></div>
